...
public interface SubCategoryMapper extends SabbMapper<SubCategory> {
	...
	@Insert("INSERT INTO sabb.sub_category(name,id_category) VAlUES(#{name}, #{category.id})")
	void insert(SubCategory subCategory);

	@Delete("DELETE FROM sabb.sub_category WHERE id=#{id}")
	void delete(SubCategory subCategory);

	@Update("UPDATE sabb.sub_category SET name=#{name}, id_category=#{category.id} WHERE id=#{id}")
