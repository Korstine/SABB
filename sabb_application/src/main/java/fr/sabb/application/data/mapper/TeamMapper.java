...
public interface TeamMapper extends SabbMapper {
	...
	@Insert("INSERT INTO sabb.team(name,id_season,id_category,id_association,active) VAlUES(#{name}, #{season.id},#{category.id},#{association.id}, #{active})")
	void insert(Team team);
	
	@Delete("DELETE FROM sabb.team WHERE id=#{id}")
	void delete(Team team);
	
	@Update("UPDATE sabb.team SET name=#{name}, active=#{active}, id_season=#{season.id},id_category=#{category.id},id_association=#{association.id} WHERE id=#{id}")
